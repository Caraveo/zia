...
Provides utilities for managing copyright headers of `The ZiaCoin Core
...
Generates a ziacoin.conf file in `share/examples/` by parsing the output from `ziacoind --help`. This script is run during the
release process to include a ziacoin.conf with the release binaries and can also be run by users to generate a file locally.
...
```bash
BUILDDIR=$PWD/my-build-dir contrib/devtools/gen-ziacoin-conf.sh
```
