# Bootstrappable ZiaCoin Core Builds

This directory contains the files necessary to perform bootstrappable ZiaCoin
Core builds.
...
To build ZiaCoin Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      ZiaCoin Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/ziacoin-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/ziacoin-core/guix.sigs).
