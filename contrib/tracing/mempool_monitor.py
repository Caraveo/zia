#!/usr/bin/env python3
# Copyright (c) 2022 The ZiaCoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

""" Example logging ZiaCoin Core mempool events using the mempool:added,
...
def main(pid):
    print(f"Hooking into ziacoind with pid {pid}")
    ziacoind_with_usdts = USDT(pid=int(pid))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    ziacoind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    ziacoind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    ziacoind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    ziacoind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[ziacoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "<pid of ziacoind>")
        exit(1)
