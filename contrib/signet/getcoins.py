#!/usr/bin/env python3
# Copyright (c) 2020-2022 The ZiaCoin Core developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing ziacoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='ziacoin-cli', help='ziacoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='ZiaCoin address to which the faucet should send')
...
parser.add_argument('ziacoin_cli_args', nargs='*', help='Arguments to pass on to ziacoin-cli (default: -signet)')

args = parser.parse_args()

if args.ziacoin_cli_args == []:
    args.ziacoin_cli_args = ['-signet']


def ziacoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.ziacoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = ziacoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = ziacoin_cli(['getnewaddress', 'faucet', 'bech32'])
