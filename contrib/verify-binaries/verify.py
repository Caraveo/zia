...
"""Script for verifying ZiaCoin Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from ziacoincore.org and ziacoin.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/ziacoin-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://ziacoincore.org"
HOST2 = "https://ziacoin.org"
VERSIONPREFIX = "ziacoin-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the ZiaCoin community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"ziacoin_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by ziacoincore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the ziacoin release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes ziacoin.org lags behind ziacoincore.org.)')
    )
