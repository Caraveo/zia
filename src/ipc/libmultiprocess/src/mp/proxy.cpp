// Copyright (c) 2019 The ZiaCoin Core developers
...
Connection::~Connection()
{
    ...
    // object, which can run arbitrary blocking ziacoin code so they have to run
    ...
    // functions run, so client objects are fully disconnected before ziacoin
    // code in the destructors are run. This way if the ziacoin code tries to
...
void Connection::addAsyncCleanup(std::function<void()> fn)
{
    ...
    //
    // In ziacoin core, running these callbacks in the right order is
