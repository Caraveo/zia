...
const char * const BITCOIN_CONF_FILENAME = "ziacoin.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //ziacoin-tx using stdin
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\ZiaCoin
    //   new: C:\Users\Username\AppData\Local\ZiaCoin
    // macOS: ~/Library/Application Support/ZiaCoin
    // Unix-like: ~/.ziacoin
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "ZiaCoin";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "ZiaCoin";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/ZiaCoin";
#else
    // Unix-like
    return pathRet / ".ziacoin";
#endif
#endif
}
