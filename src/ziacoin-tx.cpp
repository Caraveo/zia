...
static void SetupZiaCoinTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " ziacoin-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The ziacoin-tx tool is used for creating and modifying ziacoin transactions.\n\n"
                "ziacoin-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded ziacoin transaction, or with \"-create [commands]\" to create a hex-encoded ziacoin transaction.\n"
                "\n"
                "Usage: ziacoin-tx [options] <hex-tx> [commands]\n"
                "or:    ziacoin-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded ziacoin transaction
