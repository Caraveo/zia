...
void ResetArgs(ArgsManager& local_args, const std::string& strArg)
{
    ...
    vecArg.insert(vecArg.begin(), "testziacoin");
...
BOOST_AUTO_TEST_CASE(patharg)
{
    ...
    const fs::path absolute_path{"/home/user/.ziacoin"};
    ResetArgs(local_args, "-dir=/home/user/.ziacoin");
    BOOST_CHECK_EQUAL(local_args.GetPathArg("-dir"), absolute_path);

    ResetArgs(local_args, "-dir=/root/../home/user/.ziacoin");
    BOOST_CHECK_EQUAL(local_args.GetPathArg("-dir"), absolute_path);
    ...
    const fs::path relative_path{"user/.ziacoin"};
    ResetArgs(local_args, "-dir=user/.ziacoin");
    BOOST_CHECK_EQUAL(local_args.GetPathArg("-dir"), relative_path);

    ResetArgs(local_args, "-dir=somewhere/../user/.ziacoin");
    BOOST_CHECK_EQUAL(local_args.GetPathArg("-dir"), relative_path);
    ...
    // line arguments or ziacoin.conf values. Currently the -dir= case cannot be
