// Copyright (c) 2023 The ZiaCoin Core developers
...
void IpcSocketTest(const fs::path& datadir)
{
    ...
    BOOST_CHECK_THROW(process->bind(datadir, "test_ziacoin", invalid_bind), std::invalid_argument);
    BOOST_CHECK_THROW(process->connect(datadir, "test_ziacoin", invalid_bind), std::invalid_argument);

    auto bind_and_listen{[&](const std::string& bind_address) {
        std::string address{bind_address};
        int serve_fd = process->bind(datadir, "test_ziacoin", address);
    ...
    auto connect_and_test{[&](const std::string& connect_address) {
        ...
        int connect_fd{process->connect(datadir, "test_ziacoin", address)};
    ...
    //   Address 'unix' path '"/tmp/test_common_ZiaCoin Core/ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff/test_ziacoin.sock"' exceeded maximum socket path length
    std::vector<std::string> addresses{
        strprintf("unix:%s", TempPath("ziacoin_sock0_XXXXXX")),
        strprintf("unix:%s", TempPath("ziacoin_sock1_XXXXXX")),
    };
