// Copyright (c) 2011-2022 The ZiaCoin Core developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <ziacoin-build-config.h> // IWYU pragma: keep

#include <qt/ziacoingui.h>

#include <qt/ziacoinunits.h>
...
const std::string ZiaCoinGUI::DEFAULT_UIPLATFORM =
...
ZiaCoinGUI::ZiaCoinGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &ZiaCoinGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &ZiaCoinGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &ZiaCoinGUI::showModalOverlay);
...
void ZiaCoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a ZiaCoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and ziacoin: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &ZiaCoinGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &ZiaCoinGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &ZiaCoinGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your ZiaCoin addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified ZiaCoin addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed ZiaCoin Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed ZiaCoin Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a ziacoin: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible ZiaCoin command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &ZiaCoinGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &ZiaCoinGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &ZiaCoinGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &ZiaCoinGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &ZiaCoinGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &ZiaCoinGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &ZiaCoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &ZiaCoinGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &ZiaCoinGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &ZiaCoinGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &ZiaCoinGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &ZiaCoinGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &ZiaCoinGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &ZiaCoinGUI::showDebugWindow);
}

void ZiaCoinGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void ZiaCoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &ZiaCoinGUI::setCurrentWalletBySelectorIndex);
...
void ZiaCoinGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &ZiaCoinGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &ZiaCoinGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &ZiaCoinGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &ZiaCoinGUI::showProgress);
...
#ifdef ENABLE_WALLET
void ZiaCoinGUI::enableHistoryAction(bool privacy)
{
...
void ZiaCoinGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &ZiaCoinGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &ZiaCoinGUI::removeWallet);
...
WalletController* ZiaCoinGUI::getWalletController()
{
    return m_wallet_controller;
}

void ZiaCoinGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &ZiaCoinGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &ZiaCoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &ZiaCoinGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &ZiaCoinGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &ZiaCoinGUI::incomingTransaction);
    connect(this, &ZiaCoinGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void ZiaCoinGUI::removeWallet(WalletModel* walletModel)
{
...
void ZiaCoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void ZiaCoinGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void ZiaCoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void ZiaCoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void ZiaCoinGUI::createTrayIcon()
{
...
void ZiaCoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &ZiaCoinGUI::toggleHidden);
...
void ZiaCoinGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void ZiaCoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void ZiaCoinGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void ZiaCoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void ZiaCoinGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void ZiaCoinGUI::openClicked()
{
...
void ZiaCoinGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void ZiaCoinGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void ZiaCoinGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void ZiaCoinGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void ZiaCoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void ZiaCoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void ZiaCoinGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void ZiaCoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to ZiaCoin network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void ZiaCoinGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void ZiaCoinGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void ZiaCoinGUI::updateHeadersSyncProgressLabel()
{
...
void ZiaCoinGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void ZiaCoinGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &ZiaCoinGUI::quitRequested);
...
void ZiaCoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void ZiaCoinGUI::createWallet()
{
#ifdef ENABLE_WALLET
    auto activity = new CreateWalletActivity(getWalletController(), this);
    connect(activity, &CreateWalletActivity::created, this, &ZiaCoinGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void ZiaCoinGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void ZiaCoinGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &ZiaCoinGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &ZiaCoinGUI::show);
                e->ignore();
            }
        }
    }
...
void ZiaCoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void ZiaCoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void ZiaCoinGUI::incomingTransaction(const QString& date, ZiaCoinUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(ZiaCoinUnits::formatWithUnit(unit, amount, true));
...
void ZiaCoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void ZiaCoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool ZiaCoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool ZiaCoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void ZiaCoinGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void ZiaCoinGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void ZiaCoinGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void ZiaCoinGUI::updateProxyIcon()
{
    std::string ip_port;
...
void ZiaCoinGUI::updateWindowTitle()
{
...
void ZiaCoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void ZiaCoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void ZiaCoinGUI::detectShutdown()
{
...
void ZiaCoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void ZiaCoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(ZiaCoinGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = ZiaCoinGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void ZiaCoinGUI::subscribeToCoreSignals()
{
...
void ZiaCoinGUI::unsubscribeFromCoreSignals()
{
...
bool ZiaCoinGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<ZiaCoinUnit> units = ZiaCoinUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const ZiaCoinUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, ZiaCoinUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const ZiaCoinUnit u : ZiaCoinUnits::availableUnits()) {
        menu->addAction(ZiaCoinUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(ZiaCoinUnit newUnits)
{
    setText(ZiaCoinUnits::longName(newUnits));
}
