...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaType<ZiaCoinUnit>("ZiaCoinUnit");
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. ziacoin_de.qm (shortcut "de" needs to be defined in ziacoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. ziacoin_de_DE.qm (shortcut "de_DE" needs to be defined in ziacoin.qrc)
...
static const char* qt_argv = "ziacoin-qt";

ZiaCoinApplication::ZiaCoinApplication()
...
void ZiaCoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the ZiaCoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", ZiaCoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void ZiaCoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool ZiaCoinApplication::createOptionsModel(bool resetSettings)
{
...
void ZiaCoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new ZiaCoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &ZiaCoinGUI::quitRequested, this, &ZiaCoinApplication::requestShutdown);
...
void ZiaCoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void ZiaCoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool ZiaCoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void ZiaCoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &ZiaCoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &ZiaCoinApplication::handleRunawayException);
    connect(this, &ZiaCoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &ZiaCoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void ZiaCoinApplication::parameterSetup()
{
...
void ZiaCoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void ZiaCoinApplication::requestInitialize()
{
...
void ZiaCoinApplication::requestShutdown()
{
...
void ZiaCoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // ziacoin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &ZiaCoinGUI::handlePaymentRequest);
            connect(window, &ZiaCoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void ZiaCoinApplication::handleRunawayException(const QString &message)
{
...
void ZiaCoinApplication::handleNonFatalException(const QString& message)
{
...
WId ZiaCoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool ZiaCoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", ZiaCoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(ziacoin);
    Q_INIT_RESOURCE(ziacoin_locale);
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see ziacoin-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see ziacoin-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse ziacoin.conf, determine network, switch to network specific
    ...
    // ziacoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
