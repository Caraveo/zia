...
/** Call node.updateRwSetting() with ZiaCoin 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because ZiaCoin 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/ziacoin/ziacoin/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because ziacoin 22.x will not try to read these.
...
//! Convert enabled/size values to ziacoin -prune setting.
...
//! Get pruning enabled value to show in GUI from ziacoin -prune setting.
...
//! Get pruning size value to show in GUI from ziacoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayZiaCoinUnit")) {
        settings.setValue("DisplayZiaCoinUnit", QVariant::fromValue(ZiaCoinUnit::ZIA));
    }
    QVariant unit = settings.value("DisplayZiaCoinUnit");
    if (unit.canConvert<ZiaCoinUnit>()) {
        m_display_ziacoin_unit = unit.value<ZiaCoinUnit>();
    } else {
        m_display_ziacoin_unit = ZiaCoinUnit::ZIA;
        settings.setValue("DisplayZiaCoinUnit", QVariant::fromValue(m_display_ziacoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if ziacoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_ziacoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<ZiaCoinUnit>() == m_display_ziacoin_unit) return;
    m_display_ziacoin_unit = new_unit.value<ZiaCoinUnit>();
    QSettings settings;
    settings.setValue("DisplayZiaCoinUnit", QVariant::fromValue(m_display_ziacoin_unit));
    Q_EMIT displayUnitChanged(m_display_ziacoin_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/ziacoin/ziacoin/pull/8273
    ...
    // (https://github.com/ziacoin-core/gui/issues/567).
