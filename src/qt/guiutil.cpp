// Copyright (c) 2011-present The ZiaCoin Core developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <qt/guiutil.h>

#include <qt/ziacoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a ZiaCoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new ZiaCoinAddressEntryValidator(parent));
    widget->setCheckValidator(new ZiaCoinAddressCheckValidator(parent));
}
...
bool parseZiaCoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no ziacoin: URI
    if(!uri.isValid() || uri.scheme() != QString("ziacoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!ZiaCoinUnits::parse(ZiaCoinUnit::ZIA, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseZiaCoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseZiaCoinURI(uriInstance, out);
}

QString formatZiaCoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("ziacoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(ZiaCoinUnits::format(ZiaCoinUnit::ZIA, info.amount, false, ZiaCoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openZiaCoinConf()
{
    ...
    /* Open ziacoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "ZiaCoin.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "ZiaCoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("ZiaCoin (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for ZiaCoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "ziacoin.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("ziacoin-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a ziacoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=ZiaCoin\n";
        else
            optionFile << strprintf("Name=ZiaCoin (%s)\n", ChainTypeToString(chain));
