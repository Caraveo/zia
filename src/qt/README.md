This directory contains the source code for the ZiaCoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the ZiaCoin UI.
...
- ClientModel is used by the main application `ziacoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a ziacoin:... link from a browser).
...
* UI elements like ZiaCoinAmountField, which inherit from QWidget.
* `ziacoinstrings.cpp`: automatically generated
* `ziacoinunits.(h/cpp)`: ZIA / mZIA / etc. handling
* `callback.h`
...
**Note:** Do not change `local/ziacoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `ziacoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `ziacoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `build/bin`, and select `ziacoin-qt`
9. You're all set! Start developing, building, and debugging the ZiaCoin Core GUI
