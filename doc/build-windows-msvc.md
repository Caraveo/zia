...
This guide describes how to build ziacoind, command-line utilities, and GUI on Windows using Microsoft Visual Studio.
...
### 3. Clone ZiaCoin Repository

Clone the ZiaCoin Core repository to a directory. All build scripts and commands will run from this directory.
```
...
The ZiaCoin Core project supports the following vcpkg triplets:
- `x64-windows` (both CRT and library linkage is dynamic)
- `x64-windows-static` (both CRT and library linkage is static)

To facilitate build process, the ZiaCoin Core project provides presets, which are used in this guide.
...
To improve the build process performance, one might add the ZiaCoin repository directory to the Microsoft Defender Antivirus exclusions.
