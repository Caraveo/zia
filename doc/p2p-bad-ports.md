When ZiaCoin Core automatically opens outgoing P2P connections, it chooses
...
A malicious actor may gossip an address:port where no ZiaCoin node is listening,
or one where a service is listening that is not related to the ZiaCoin network.
As a result, this service may occasionally get connection attempts from ZiaCoin
nodes.

"Bad" ports are ones used by services which are usually not open to the public
and usually require authentication. A connection attempt (by ZiaCoin Core,
trying to connect because it thinks there is a ZiaCoin node on that
...
Below is a list of "bad" ports which ZiaCoin Core avoids when choosing a peer to
...
[pull/23306](https://github.com/ziacoin/ziacoin/pull/23306#issuecomment-947516736)

[pull/23542](https://github.com/ziacoin/ziacoin/pull/23542)
