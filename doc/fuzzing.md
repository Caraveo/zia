# Fuzzing ZiaCoin Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing ZiaCoin Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/ziacoin/ziacoin
$ cd ziacoin/
$ cmake --preset=libfuzzer
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/ziacoin/ziacoin/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
...
## Overview of ZiaCoin Core fuzzing

[Google](https://github.com/google/fuzzing/) has a good overview of fuzzing in general, with contributions from key architects of some of the most-used fuzzers. [This paper](https://agroce.github.io/ziacoin_report.pdf) includes an external overview of the status of ZiaCoin Core fuzzing, as of summer 2021.  [John Regehr](https://blog.regehr.org/archives/1687) provides good advice on writing code that assists fuzzers in finding bugs, which is useful for developers to keep in mind.

## Fuzzing harnesses and output

[`process_message`](https://github.com/ziacoin/ziacoin/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/ziacoin/ziacoin/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/ziacoin/ziacoin/tree/master/src/test/fuzz).
...
It is possible to specify `ziacoind` arguments to the `fuzz` executable.
...
The project's collection of seed corpora is found in the [`ziacoin-core/qa-assets`](https://github.com/ziacoin-core/qa-assets) repo.

To fuzz `process_message` using the [`ziacoin-core/qa-assets`](https://github.com/ziacoin-core/qa-assets) seed corpus:

```sh
$ git clone --depth=1 https://github.com/ziacoin-core/qa-assets
...
an instrumentalized libc++, then using it to build [ZiaCoin Core dependencies
from source](../depends/README.md) and finally the ZiaCoin Core fuzz binary
...
the qa-assets repo
(https://github.com/ziacoin-core/qa-assets/blob/main/.github/PULL_REQUEST_TEMPLATE.md).
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`ziacoin-core/qa-assets`](https://github.com/ziacoin-core/qa-assets) repo.

Every single pull request submitted against the ZiaCoin Core repo is automatically tested against all inputs in the [`ziacoin-core/qa-assets`](https://github.com/ziacoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make ZiaCoin Core more robust.
...
# Fuzzing ZiaCoin Core using afl++

## Quickstart guide

To quickly get started fuzzing ZiaCoin Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing ZiaCoin Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing ZiaCoin Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
ZiaCoin Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/ziacoin-core)
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://issues.oss-fuzz.com/issues?q=ziacoin-core%20status:open).

ZiaCoin Core follows its [security disclosure policy](https://ziacoincore.org/en/security-advisories/),
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_ziacoin-core/latest).
