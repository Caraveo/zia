...
Below are some notes on how to build ZiaCoin Core for Windows.

The options known to work for building ZiaCoin Core on Windows are:
...
Note that for WSL the ZiaCoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/ziacoin, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\ziacoin`, for example:
```shell
cmake --install build --prefix /mnt/c/workspace/ziacoin
```
...
```shell
cmake --install build --prefix /mnt/c/workspace/ziacoin --strip
```
