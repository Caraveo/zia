# CJDNS support in ZiaCoin Core

It is possible to run ZiaCoin Core over CJDNS, an encrypted IPv6 network that
...
done outside of ZiaCoin Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the ZiaCoin network and individual nodes.
...
before it will work with your ZiaCoin Core node. This is described in steps
...
## Run ZiaCoin Core with CJDNS

Once you are connected to the CJDNS network, the following ZiaCoin Core
...
When enabled, this option tells ZiaCoin Core that it is running in an
...
IPv6 local network. This helps ZiaCoin Core perform better address management:
...
CJDNS support was added to ZiaCoin Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `ziacoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/ziacoin/ziacoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in ZiaCoin Core

There are several ways to see your CJDNS address in ZiaCoin Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `ziacoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `ziacoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `ziacoin-cli help getnodeaddresses` for details.
