# User-space, Statically Defined Tracing (USDT) for ZiaCoin Core

ZiaCoin Core includes statically defined tracepoints to allow for more
...
**Note**: ZiaCoin Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to ZiaCoin Core
...
Multiple tools can list the available tracepoints in a `ziacoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in ZiaCoin Core, use `info probes` in `gdb`:

```
$ gdb ./build/bin/ziacoind
…
(gdb) info probes
Type Provider   Name             Where              Semaphore Object
stap net        inbound_message  0x000000000014419e 0x0000000000d29bd2 /build/bin/ziacoind
stap net        outbound_message 0x0000000000107c05 0x0000000000d29bd0 /build/bin/ziacoind
stap validation block_connected  0x00000000002fb10c 0x0000000000d29bd8 /build/bin/ziacoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in ZiaCoin Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./build/bin/ziacoind | grep NT_STAPSDT -A 4 -B 2
