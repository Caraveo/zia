...
random value payments which sum to 10ZIA for each output type to Bob,
for a total of 40ZIA in Bob's wallet.
...
class AddressInputTypeGrouping(ZiaCoinTestFramework):
    ...
    def make_payment(self, A, B, v, addr_type):
        ...
        self.log.debug(f"Making payment of {v} ZIA at fee_rate {fee_rate}")
        tx = B.sendtoaddress(
    ...
    def run_test(self):
        ...
        for v in generate_payment_values(3, 10):
            self.log.debug(f"Making payment of {v} ZIA to legacy")
            A.sendtoaddress(B.getnewaddress(address_type="legacy"), v)

        for v in generate_payment_values(3, 10):
            self.log.debug(f"Making payment of {v} ZIA to p2sh")
            A.sendtoaddress(B.getnewaddress(address_type="p2sh-segwit"), v)

        for v in generate_payment_values(3, 10):
            self.log.debug(f"Making payment of {v} ZIA to bech32")
            A.sendtoaddress(B.getnewaddress(address_type="bech32"), v)

        for v in generate_payment_values(3, 10):
            self.log.debug(f"Making payment of {v} ZIA to bech32m")
