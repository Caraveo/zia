#!/usr/bin/env python3
# Copyright (c) 2014-present The ZiaCoin Core developers
...
TMPDIR_PREFIX = "ziacoin_func_test_"
...
class Binaries:
    """Helper class to provide information about ziacoin binaries
    ...
    def daemon_argv(self):
        "Return argv array that should be used to invoke ziacoind"
        return self._argv(self.paths.ziacoind)

    def rpc_argv(self):
        "Return argv array that should be used to invoke ziacoin-cli"
        return self._argv(self.paths.ziacoincli)

    def util_argv(self):
        "Return argv array that should be used to invoke ziacoin-util"
        return self._argv(self.paths.ziacoinutil)

    def wallet_argv(self):
        "Return argv array that should be used to invoke ziacoin-wallet"
        return self._argv(self.paths.ziacoinwallet)

    def chainstate_argv(self):
        "Return argv array that should be used to invoke ziacoin-chainstate"
        return self._argv(self.paths.ziacoinchainstate)
...
class ZiaCoinTestMetaClass(type):
    """Metaclass for ZiaCoinTestFramework.

    Ensures that any attempt to register a subclass of `ZiaCoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'ZiaCoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("ZiaCoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("ZiaCoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class ZiaCoinTestFramework(metaclass=ZiaCoinTestMetaClass):
    """Base class for a ziacoin test script.

    Individual ziacoin test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave ziacoinds and test.* datadir on exit or error")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use ziacoin-cli instead of RPC for all commands")
    ...
    def get_binary_paths(self):
        ...
        binaries = {
            "ziacoind": ("ziacoind", "BITCOIND"),
            "ziacoin-cli": ("ziacoincli", "BITCOINCLI"),
            "ziacoin-util": ("ziacoinutil", "BITCOINUTIL"),
            "ziacoin-chainstate": ("ziacoinchainstate", "BITCOINCHAINSTATE"),
            "ziacoin-wallet": ("ziacoinwallet", "BITCOINWALLET"),
        }
    ...
    def shutdown(self):
        ...
        rpc_logger = logging.getLogger("ZiaCoinRPC")
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a ziacoind test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple ziacoind test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the ziacoind
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as ziacoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in ziacoin.conf
    ...
    def skip_if_no_ziacoind_tracepoints(self):
        """Skip the running test if ziacoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("ziacoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_ziacoind_zmq(self):
        """Skip the running test if ziacoind has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("ziacoind has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if ziacoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("ziacoin-wallet has not been compiled")

    def skip_if_no_ziacoin_util(self):
        """Skip the running test if ziacoin-util has not been compiled."""
        if not self.is_ziacoin_util_compiled():
            raise SkipTest("ziacoin-util has not been compiled")

    def skip_if_no_ziacoin_chainstate(self):
        """Skip the running test if ziacoin-chainstate has not been compiled."""
        if not self.is_ziacoin_chainstate_compiled():
            raise SkipTest("ziacoin-chainstate has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if ziacoin-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("ziacoin-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether ziacoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether ziacoin-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_ziacoin_util_compiled(self):
        """Checks whether ziacoin-util was compiled."""
        return self.config["components"].getboolean("ENABLE_BITCOIN_UTIL")

    def is_ziacoin_chainstate_compiled(self):
        """Checks whether ziacoin-chainstate was compiled."""
