...
# Copyright (c) 2010-2022 The ZiaCoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.
"""ZiaCoin test framework primitive and message structures
...
# Objects that map to ziacoind objects, which can be serialized/deserialized


class CAddress:
    __slots__ = ("net", "ip", "nServices", "port", "time")

    # see https://github.com/ziacoin/bips/blob/master/bip-0155.mediawiki
    NET_IPV4 = 1
    NET_IPV6 = 2
    NET_TORV3 = 4
    NET_I2P = 5
...
class CBlockLocator:
    ...
    def serialize(self):
        r = b""
        r += (0).to_bytes(4, "little", signed=True)  # ZiaCoin Core ignores the version field. Set it to 0.
...
class CTransaction:
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in ziacoind
            if (flags != 0):
                self.vin = deser_vector(f, CTxIn)
...
class msg_version:
    ...
    def deserialize(self, f):
        ...
        # But, unconditionally check it to match behaviour in ziacoind
...
class msg_headers:
    ...
    def deserialize(self, f):
        # comment in ziacoind indicates these should be deserialized as blocks
