#!/usr/bin/env python3
# Copyright (c) 2014-2022 The ZiaCoin Core developers
...
class RawTransactionsTest(ZiaCoinTestFramework):
    ...
    def test_invalid_change_address(self):
        ...
        assert_raises_rpc_error(-5, "Change address must be a valid ziacoin address", self.nodes[2].fundrawtransaction, rawtx, changeAddress='foobar')
    ...
    def test_option_feerate(self):
        self.log.info("Test fundrawtxn with explicit fee rates (fee_rate sat/vB and feeRate ZIA/kvB)")
        node = self.nodes[3]
        ...
        assert_raises_rpc_error(-8, "Cannot specify both fee_rate (sat/vB) and feeRate (ZIA/kvB)",
    ...
    def test_option_subtract_fee_from_outputs(self):
        ...
        # Test subtract fee from outputs with feeRate (ZIA/kvB)
    ...
    def test_transaction_too_large(self):
        ...
        # Make 1500 0.1 ZIA outputs. The amount that we target for funding is in
    ...
    def test_add_inputs_default_value(self):
        ...
        # Create and fund the wallet with 5 ZIA
        ...
        # Fund wallet with 2 outputs, 5 ZIA each.
    ...
    def test_preset_inputs_selection(self):
        ...
        # Create and fund the wallet with 4 UTXO of 5 ZIA each (20 ZIA total)
        ...
        # Attempt to send 29 ZIA from a wallet that only has 20 ZIA. The wallet should exclude
        # the preset inputs from the pool of available coins, realize that there is not enough
        # money to fund the 29 ZIA payment, and fail with "Insufficient funds".
        #
        # Even with SFFO, the wallet can only afford to send 20 ZIA.
        # If the wallet does not properly exclude preset inputs from the pool of available coins
        # prior to coin selection, it may create a transaction that does not fund the full payment
        # amount or, through SFFO, incorrectly reduce the recipient's amount by the difference
        # between the original target and the wrongly counted inputs (in this case 9 ZIA)
        # so that the recipient's amount is no longer equal to the user's selected target of 29 ZIA.
    ...
    def test_22670(self):
        ...
        # that this bug is not detected because the transaction fee must be at least 0.01 ZIA (the minimum change value).
        # Otherwise the targeted minimum change value will be enough to cover the transaction fees that were not
        # being accounted for. So the minimum relay fee is set to 0.1 ZIA/kvB in this test.
    ...
    def test_feerate_rounding(self):
        ...
        # fail with insufficient funds rather than ziacoind asserting.
    ...
    def test_input_confs_control(self):
        ...
        # Create a replacement tx to 'final_tx1' that has 1 ZIA target instead of 0.1.
        ...
        # Now fund 'raw_tx2' to fulfill the total target (1 ZIA) by using all the wallet unconfirmed outputs.
        # As it was created with the first unconfirmed output, 'raw_tx2' only has 0.1 ZIA covered (need to fund 0.9 ZIA more).
